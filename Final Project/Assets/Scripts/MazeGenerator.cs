...
public class MazeGenerator : MonoBehaviour
{
    ...
    //found at https://github.com/c00pala/Unity-2D-Maze-Generator
    ...
    private void RepeateBackground()
    {
        AddCellsVistied();
        GenerateMaze2(mazeRows, mazeColumns);
       
    }

    private void AddCellsVistied() {

        while (visited.Count > 0)
        {

            List<Cell> visitedNeighbours = GetVisitedNeighbours(currentCell);
           
                // Get a random unvisited neighbour.
                checkCell = visited[Random.Range(0, visitedNeighbours.Count)];
                //// Add current cell to stack.
                //stack.Add(currentCell);
                //// Compare and remove walls.
                CompareWalls2(currentCell, checkCell);
    ...
    public void CompareWalls2(Cell cCell, Cell nCell)
    {
        AddWall(nCell.cScript, 2);
        AddWall(cCell.cScript, 1);
        AddWall(nCell.cScript, 3);
        AddWall(cCell.cScript, 4);
       
    }
    public void AddWall(CellScript cScript, int wallID)
    {
        if (wallID == 1) cScript.wallL.SetActive(true);
        else if (wallID == 2) cScript.wallR.SetActive(true);
        else if (wallID == 3) cScript.wallU.SetActive(true);
        else if (wallID == 4) cScript.wallD.SetActive(true);
    }

    private void GenerateMaze2(int rows, int columns)
    {
    ...
    public void CreateLayout2()
    {
        ...
        Vector2 BGoffSet = new Vector2(0, backgroundSize);
        GameObject.Find("Maze1").transform.position = (Vector2)transform.position + BGoffSet;
        Vector2 startPos = GameObject.Find("Maze1").transform.position;
    ...
    public void CreateLayout()
    {
        ...
        Vector2 startPos = GameObject.Find("Maze1").transform.position;
        //new Vector2(-(cellSize * (mazeColumns / 2)) + (cellSize / 2), -(cellSize * (mazeRows / 2)) + (cellSize / 2 ));
        Vector2 spawnPos = startPos;
    ...
    public void MakeExit()
    {
        ...
        GameObject.Find("Maze1").transform.Rotate(90,0,0);
        GameObject.Find("Maze1").transform.position = new Vector3(0f, -0f, 0f);
    ...
    public List<Cell> GetVisitedNeighbours(Cell curCell)
    {
        // Create a list to return.
        List<Cell> neighbours = new List<Cell>();
        // Create a Cell object.
        ...
        foreach (Vector2 p in neighbourPositions)
        {
            ...
            if (allCells.ContainsKey(nPos)) nCell = allCells[nPos];
            // If cell is unvisited.
            if (visited.Contains(nCell)) neighbours.Add(nCell);
    ...
    public void CreateCentre()
    {
        ...
        List<int> rndList = new List<int> {2, 3 };
    ...
    public void InitValues()
    {
        ...
        mazeParent = GameObject.Find("Maze1");
